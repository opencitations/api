...
class TextSearch():
    ...
    def get_text_search_on_id(self, ts_index:bool) -> str:
        ...
        return '''
                {0}
                ?tsIdentifier{1} literal:hasLiteralValue ?tsId{1};
                                        datacite:usesIdentifierScheme datacite:{2}.
                ?res datacite:hasIdentifier ?tsIdentifier{1};
                    a fabio:Expression.
        '''.format(
                self.__gen_text_search('tsId{}'.format(ts_index), literal_value, True, ts_index),
                ts_index,
                schema
        )
    ...
    def get_text_search_on_title(self, ts_index:bool) -> str:
        return '''
            ...
            ?res dcterm:title ?tsTitle{};
                a fabio:Expression.
        '''.format(self.__gen_text_search('tsTitle{}'.format(ts_index), self.text, False, ts_index), ts_index)
        ...
        if ',' in clean_test:
            ...
            if name_parts:
                ...
                if len(name_parts) == 2:
                    ...
                    given_name = ' '.join(["{0}.+?".format(name_part.rstrip('.')) if len(name_part.rstrip('.')) == 1 else name_part for name_part in given_name.split()])
        ...
        if name:
            ...
            if family_name and given_name:
                base_query = '''
                    ...
                    FILTER(REGEX(?ts{role}Gn{ts_index}, '{given_name}'))
                '''.format(
                    role=role,
                    ts_index=ts_index,
                ...
                base_query = "?ts{role}Ra{ts_index} foaf:familyName '{family_name}'.".format(
                    role=role,
                    ts_index=ts_index,
                    family_name=family_name
                ) + base_query
                ...
                base_query = "?ts{role}Ra{ts_index} foaf:givenName '{given_name}'.".format(
                    role=role,
                    ts_index=ts_index,
                    given_name=given_name
                ) + base_query
        ...
        if close_match:
            ...
            text_search_on_publisher = '''
                ...
                ?res pro:isDocumentContextFor ?tsPublisher{ts_index};
                    a fabio:Expression.
            '''.format(ts_index=ts_index, publisher=publisher)
            ...
            text_search_on_publisher = '''
                ...
                ?res pro:isDocumentContextFor ?tsPublisher{ts_index};
                    a fabio:Expression.
            '''.format(
                self.__gen_text_search('tsPublisherName{}'.format(ts_index), self.text, False, ts_index),
                ts_index
            )
    ...
    def get_text_search_on_vi(self, vi:str, ts_index:bool) -> str:
        v_or_i = vi.title()
        return f'''
            {self.__gen_text_search('ts{}Number{}'.format(v_or_i, ts_index), self.text, False, ts_index)}
            ...
            ?res frbr:partOf+ ?ts{v_or_i}{ts_index};
                a fabio:Expression.
        '''.format(v_or_i, ts_index)

    def get_text_search_on_venue(self, ts_index:bool) -> str:
        return '''
            {0}
            ?tsVenue{1} dcterm:title ?tsVenueTitle{1}.
            ?res frbr:partOf+ ?tsVenue{1}.
            FILTER NOT EXISTS {{?res a fabio:JournalVolume}}
            FILTER NOT EXISTS {{?res a fabio:JournalIssue}}
        '''.format(self.__gen_text_search('tsVenueTitle{}'.format(ts_index), self.text, False, ts_index), ts_index)

    def __gen_text_search(self, variable:str, text:str, perfect_match:bool, ts_index:int) -> str:
        if str(ts_index).startswith('0'):
            min_relevance = "bds:minRelevance '0.6'; bds:matchAllTerms 'true'." if not perfect_match else "bds:matchRegex '^{}$'.".format(text)
            text_search = "?{} bds:search '{}'; {}".format(variable, text, min_relevance)
        else:
            pattern = '^{}$'.format(text) if perfect_match else text
            text_search = "FILTER REGEX (?{}, '{}', 'i')".format(variable, pattern)
        return text_search
